<template>
	<view class="container">
		...
		<view :animation="animationData" class="region-box" v-show="showModalStatus">
			<view class="region-txt"  v-for="(item,index) in regionArr" :key='index' :class="[(index==regionArr.length-3 || index==regionArr.length-2 || index==regionArr.length-1)?'grow':'',tabIndex==index?'active':'']"
			 :data-index="index" @tap.stop="getRegion">{{item}}</view>
		</view>
